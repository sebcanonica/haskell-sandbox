...
addUser userName =
    withConn "tools.db" $ \conn -> do
        execute conn "INSERT INTO users (username) VALUES (?)"
            (Only userName) -- use classic tuple if more than 1 param
        print "user added"
...
promptAndAddTool = do
   ...
   currentDay <- utctDay <$> getCurrentTime
   addTool toolName toolDescription currentDay
